...
{
    private float moveDistance = 1f;    // 한번에 이동하는 거리
    ...
    private void FixedUpdate()
    {
        if (shouldMove) // 움직여야하는 상태면
        {
            ...
            if (Vector3.Distance(transform.position, targetPosition) < 0.01f)   // 목표 지점에 근접하면
            {
